...
private:
	...
	simpleLogger()
	{
		...
		const std::string logger_name_prefix = "log_";
	...
	simpleLogger(const simpleLogger&) = delete;  // 不能被调用的成员函数
	simpleLogger& operator=(const simpleLogger&) = delete; // 不能被调用的成员函数
...
#define SLOG_TRACE(...) SPDLOG_LOGGER_CALL(simpleLogger::getInstance()->getLogger().get(), spdlog::level::trace, __VA_ARGS__)
#define SLOG_DEBUG(...) SPDLOG_LOGGER_CALL(simpleLogger::getInstance()->getLogger().get(), spdlog::level::debug, __VA_ARGS__)
#define SLOG_INFO(...) SPDLOG_LOGGER_CALL(simpleLogger::getInstance()->getLogger().get(), spdlog::level::info, __VA_ARGS__)
#define SLOG_WARN(...) SPDLOG_LOGGER_CALL(simpleLogger::getInstance()->getLogger().get(), spdlog::level::warn, __VA_ARGS__)
#define SLOG_ERROR(...) SPDLOG_LOGGER_CALL(simpleLogger::getInstance()->getLogger().get(), spdlog::level::err, __VA_ARGS__)
